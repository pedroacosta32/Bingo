...
function drawBall(){
    ...
    //Display draw ball
    pdrawBall.innerHTML = ballDraw.ballDrawLetter+" - " + ballDraw.ballDrawnumber;

    //loop through the bingo cards numbers
    for ( var i = 0; i < cardMatrixTotal.length; i++) {
        for ( var j = 0; j < cardMatrixTotal.length; j++) {
            for ( var k = 0; k < cardMatrixTotal[i][j].length; k++) {

                //highlights the numbers matches for drawball into the bingo card
                if (cardMatrixTotal[i][j][k] == ballDraw.ballDrawnumber) {
...
function showWinnerCard() {
    //user confirmation to display bingo card (winning card)
...
//Clear or restart BINGO card
function clearCards() {
    //user confirmation before restarting
    let confirmation = confirm( "Do You Want to Clear / Reset game?");
    if (confirmation = true) {
        //restart
...
function checkBingoWinner(){
    ...
    // check if bingo winner is diagonal
        ...
        console.log ( `No winner Bingo Yet`);
    }
}
